...
void backlightOn() {
  #if defined(HAS_SCREEN) && defined(TFT_BL) && TFT_BL >= 0
    #ifdef MARAUDER_MINI
...
void setup()
{
  ...
  Serial.begin(115200);
  // No while(!Serial) — UART0 is always ready; that guard is only needed for USB CDC

  Serial.println("ESP-IDF version is: " + String(esp_get_idf_version()));
  Serial.println("DBG: before RunSetup");

  #ifdef HAS_SCREEN
    display_obj.RunSetup();
    Serial.println("DBG: RunSetup done");
  ...
  backlightOff();
    ...
    display_obj.tft.drawCentreString("K4eru", TFT_WIDTH / 2, TFT_HEIGHT * 0.5, 1);
  ...
  #endif
  Serial.println("DBG: battery done");
  ...
  #ifdef HAS_SCREEN
    Serial.println("DBG: before menu RunSetup");
    menu_function_obj.RunSetup();
    Serial.println("DBG: menu RunSetup done");
  #endif

  Serial.println("DBG: before StartScan");
